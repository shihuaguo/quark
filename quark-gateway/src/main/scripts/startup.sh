...
SERVICE_NAME=quark-gateway-server
export APP_NAME=quark-gateway-server
...
function checkPidAlive {
    ...
    if [[ ! -f "$pid_file" ]]; then
        ...
        printf "pid_file is $pid_file \n"
    fi
    printf "pid_file is $pid_file\n"
    for i in `ls -t $pid_file 2>/dev/null`
    do
        read pid < $i
